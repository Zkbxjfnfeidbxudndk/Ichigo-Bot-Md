...
let handler = async (m, { conn, command, usedPrefix, args, text, isOwner }) => {
  ...
  switch (true) {       
    case isCommand1:
      ...
      const jadi = 'jadi' // تأكد من تعريف متغير jadi أو عدل حسب المطلوب
      ...
      if (global.conn.user.jid !== conn.user.jid) {
        return conn.sendMessage(m.chat, {text: ` Use este comando al *Bot* principal.\n\n*https://api.whatsapp.com/send/?phone=${global.conn.user.jid.split`@`[0]}&text=${usedPrefix + command}&type=phone_number&app_absent=0*`}, { quoted: m }) 
      } else {
    ...
    case isCommand2:
      if (global.conn.user.jid == conn.user.jid) {
        conn.reply(m.chat, ` El Bot principal no se puede apagar.`, m)
      } else {
        await conn.reply(m.chat, ` Adiós GokuBlack`, m)
        conn.ws.close()
      }
      break
...
handler.command = ['deletesesion', 'deletebot', 'deletesession', 'deletesesaion', 'stop', 'pausargokublack', 'pausarbot', 'bots', 'listjadibots', 'subbots']
